...
export TERRAFORM_PROVIDER_VERSION ?= 0.12.0
...
xpkg.build.provider-b2: do.build.images
...
crddiff: $(UPTEST)
	...
	@for crd in $${MODIFIED_CRD_LIST}; do \
		...
		changes_detected=$$(go run github.com/crossplane/uptest/cmd/crddiff@$(CRDDIFF_VERSION) revision --enable-upjet-extensions <(git cat-file -p "$${GITHUB_BASE[27;5;106~_REF}:$${crd}") "$${crd}" 2>&1) ; \
		if [[ $$? != 0 ]] ; then \
