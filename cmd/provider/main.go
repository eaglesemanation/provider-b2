...
import (
	...
	apisCluster "github.com/eaglesemanation/provider-b2/apis/cluster"
	apisNamespaced "github.com/eaglesemanation/provider-b2/apis/namespaced"
	"github.com/eaglesemanation/provider-b2/config"
	"github.com/eaglesemanation/provider-b2/internal/clients"
	controllerCluster "github.com/eaglesemanation/provider-b2/internal/controller/cluster"
	controllerNamespaced "github.com/eaglesemanation/provider-b2/internal/controller/namespaced"
	"github.com/eaglesemanation/provider-b2/internal/features"
	"github.com/eaglesemanation/provider-b2/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for Backblaze B2").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-b2"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-b2",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped Backblaze B2 APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced Backblaze B2 APIs to scheme")
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped Backblaze B2 controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced Backblaze B2 controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped Backblaze B2 controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced Backblaze B2 controllers")
	}
